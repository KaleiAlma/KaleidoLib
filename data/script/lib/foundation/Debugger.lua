...
---@class foundation.debugger.View
local W = {}
...
---@type foundation.debugger.ViewCollection.Item[]
local view_collection = {}

---@param id any
---@param view foundation.debugger.View
function M.addView(id, view)
    ...
    ---@class foundation.debugger.ViewCollection.Item
    local t = {
        id = id,
        view = view,
    }
...
function M.KeyDownTrigger(vkey)
    ...
    local _state = false
    return function ()
        _state = lstg.Input.Keyboard.GetKeyState(vkey)
...
local function changeGpu()
    if string.len(select_gpu) > 0 then
        ...
        ScreenResources.refreshScreenResources()
        select_gpu = ""
    end
end
local function showSelectGpuWindow(enable)
    local ImGui = imgui.ImGui
    if #gpu_list < 1 then
        gpu_list = lstg.EnumGPUs()
    end
    local show, open = ImGui.Begin("Select GPU", enable)
    if show then
...
local b_show_paredit_window = false
...
function M.update()
    ...
    if imgui_exist then
        ...
        if b_show_all then
            ...
            -- flag = flag or b_show_paredit_window
...
function M.layout()
    ...
    if imgui_exist then
        ...
        if b_show_all then
            if b_show_menubar then
                if imgui.ImGui.BeginMainMenuBar() then
                    ...
                    if imgui.ImGui.BeginMenu("Tool") then
                        ...
                        if imgui.ImGui.MenuItem("ImGui Demo", nil, b_show_demo_window) then b_show_demo_window = not b_show_demo_window end
                        -- if imgui.ImGui.MenuItem("HGE Particle Editor", nil, b_show_paredit_window) then b_show_paredit_window = not b_show_paredit_window end
                        for _, v in ipairs(view_collection) do
            ...
            -- if b_show_paredit_window then
            --     b_show_paredit_window = imgui.backend.ShowParticleSystemEditor(b_show_paredit_window)
            -- end
